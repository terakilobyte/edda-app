...
impl CarrierPlan {
    ...
    pub fn on_jump_request(
        ...
        if next.name.eq_ignore_ascii_case(system) {
            ...
            Some(if after == 0 {
                ...
                format!(
                    "That is hop {} of {} on your carrier route; {after} more after it.",
                    self.next + 1,
                    self.hops.len()
                )
            })
            ...
            Some(format!(
                "That jump is off your carrier route, which expects {} next.",
                next.name
            ))
        }
    }
    ...
    pub fn on_arrival(&mut self, system: &str, ts: &str) -> Option<String> {
        let pos = self
            ...
            .iter()
            .skip(self.next)
            .position(|h| h.name.eq_ignore_ascii_case(system))?;
        let arrived = self.next + pos;
        if let Some(s) = self.scheduled.take() {
            if let (Ok(a), Ok(b)) = (
                chrono::DateTime::parse_from_rfc3339(&s.requested_ts),
        ...
        if let Verdict::ShortBy { tons, at_hop } = &self.verdict {
            if *at_hop as usize == self.next + 1 {
                line.push_str(&format!(
                    " The plan runs {tons} tonnes short on this jump: refuel first."
                ));
            }
        }
        Some(line)
...
pub fn load(conn: &Connection) -> Option<CarrierPlan> {
    conn.query_row("SELECT json FROM carrier_route WHERE id = 1", [], |r| {
        r.get::<_, String>(0)
    })
    .optional()
    .ok()
...
pub fn on_event(
    ...
    conn: &Connection,
    v: &Value,
    out: &mut Vec<(Callout, Option<Value>)>,
) {
...
pub fn plot(
    ...
    let carrier = carriers
        .iter()
        .find(|c| c.owned && !c.decommissioned)
        .or(carriers.first());
    let from_name = from
        .map(str::to_string)
    ...
    let tank_t = carrier
        .and_then(|c| c.tank_tritium_t.as_ref().map(|t| t.value as f32))
        .unwrap_or(ed_galaxy::carrier::TANK_T);
    let capacity_used_t = carrier
        .and_then(|c| {
            c.capacity
                .as_ref()
                .and_then(|cap| cap.value["used_t"].as_f64())
        })
    ...
    let b = galaxy
        .find(to)
        .ok_or_else(|| format!("unknown system {to:?}"))?;
    let req = ed_galaxy::carrier::CarrierRequest {
        from: a,
        to: b,
        capacity_used_t,
        tank_t,
...
#[tauri::command]
pub async fn carrier_route_plot(
    state: State<'_, AppState>,
    to: String,
    from: Option<String>,
) -> Result<CarrierRoute, String> {
    let conn = state.read_conn()?;
    let routing = state.routing.clone();
    let data_dir = state.data_dir.clone();
    tauri::async_runtime::spawn_blocking(move || {
        plot(&conn, &routing, &data_dir, &to, from.as_deref()).map(|(r, ..)| r)
    })
    .await
    .map_err(|e| e.to_string())?
}
...
pub fn plot_and_follow(state: &AppState, to: &str, from: Option<&str>) -> Result<Value, String> {
    let conn = state.read_conn()?;
    let (route, carrier_id, callsign, tank) =
        plot(&conn, &state.routing, &state.data_dir, to, from)?;
    drop(conn);
    let now = chrono::Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string();
...
#[tauri::command]
pub async fn carrier_route_start(
    state: State<'_, AppState>,
    route: CarrierRoute,
) -> Result<Value, String> {
    let now = chrono::Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string();
    let carriers = state
        .with_read(|s| ed_store::carrier::status(s.conn(), &now))
        .map_err(|e| e.to_string())?;
    let carrier = carriers
        .iter()
        .find(|c| c.owned && !c.decommissioned)
...
pub async fn carrier_route_next(state: State<'_, AppState>) -> Result<String, String> {
    let plan = state
        ...
        .ok_or("no carrier route is being followed")?;
...
mod tests {
    ...
    fn a_request_for_the_next_hop_is_ours_and_counts_the_rest() {
        let mut p = plan();
        let line = p
            .on_jump_request("Beta", Some("2026-09-07T00:30:00Z"), "2026-09-07T00:00:00Z")
            .unwrap();
        assert_eq!(
            line,
            "That is hop 1 of 3 on your carrier route; 2 more after it."
        );
        assert!(p.scheduled.is_some());
        let off = p
            .on_jump_request("Elsewhere", None, "2026-09-07T00:01:00Z")
            .unwrap();
    ...
    fn arrival_advances_measures_the_cadence_and_warns_at_the_short_hop() {
        ...
        assert_eq!(
            line,
            "2 carrier jumps remain. Next: Gamma, 470 light years, 84 tonnes of tritium."
        );
        assert_eq!(p.measured_minutes, vec![31.0]);
        assert_eq!(
            p.eta_minutes(),
            62,
            "two jumps at the measured 31 minutes, not the 20-minute floor"
        );
        let line = p.on_arrival("Gamma", "2026-09-07T01:10:00Z").unwrap();
        assert!(line.starts_with("1 carrier jump remain."), "{line}");
        assert!(
            line.contains("10 tonnes short"),
            "the plan's shortfall is at hop 3: {line}"
        );
        assert_eq!(
            p.on_arrival("Delta", "2026-09-07T01:50:00Z").unwrap(),
            "Carrier route complete: Delta reached."
        );
        assert!(p.done());
    }
    ...
    fn view_reports_the_floor_until_a_cadence_is_measured() {
        ...
        assert_eq!(
            (
                v["remaining"].as_u64(),
                v["eta_minutes"].as_u64(),
                v["cadence_measured"].as_bool()
            ),
            (Some(3), Some(60), Some(false))
        );
