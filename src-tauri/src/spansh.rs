...
pub async fn resolve_unknown_stars(state: &AppState, hops: &mut [Hop]) -> usize {
    ...
    for h in hops.iter_mut() {
        ...
        let Some(bodies) = rec.get("bodies").and_then(|b| b.as_array()) else {
            continue;
        };
        let star = bodies
            .iter()
        ...
        let Some(subtype) = star.and_then(|b| b.get("subtype")).and_then(|s| s.as_str()) else {
            continue;
        };
        ...
        let _ =
            state.with_store(|s| ed_store::stars::save(s.conn(), &star).map_err(|e| e.to_string()));
        resolved += 1;
    }
    if resolved > 0 {
...
pub fn learn_star_conn(
    ...
    let class = if source == "journal" {
        ed_galaxy::StarClass::from_journal(subtype)
    } else {
        ed_galaxy::StarClass::from_subtype(subtype)
    };
    if class == ed_galaxy::StarClass::Unknown {
        return;
    }
...
async fn resolve_via_api(
    ...
    let unknown: Vec<u64> = hops
        .iter()
        .filter(|h| h.class == ed_galaxy::StarClass::Unknown && h.id64 != 0)
        .map(|h| h.id64)
        .collect();
    let mut resolved = 0;
    for chunk in unknown.chunks(500) {
        ...
        let answer: StarsAnswer = match state
            .http
            .get(&url)
        ...
        for known in answer.known {
            ...
            for h in hops
                .iter_mut()
                .filter(|h| h.id64 == known.id64 as u64 && h.class == ed_galaxy::StarClass::Unknown)
            {
                h.class = class;
                h.scoopable = known.scoopable;
                galaxy.learn_class(h.id64, class);
                let star =
                    star_override(h.id64, Some(h.name.clone()), class.name(), class, "server");
                let _ = state.with_store(|s| {
                    ed_store::stars::save(s.conn(), &star).map_err(|e| e.to_string())
                });
                resolved += 1;
            }
        }
...
pub fn store_star_override(
    ...
    ed_store::stars::save(
        conn,
        &star_override(id64, Some(name.to_string()), subtype, class, source),
    )
}
...
pub fn backfill_journal_stars(state: &AppState) {
    {
        let rows: Vec<(u64, String, String)> = state.with_read(|s| {
            let mut stmt = match s.conn().prepare(
                "SELECT raw FROM events WHERE event = 'Scan' AND raw LIKE '%\"StarType\"%'",
            ) {
                Ok(st) => st,
            ...
            stmt.query_map([], |r| r.get::<_, String>(0))
                .map(|it| {
                    it.flatten()
                        ...
                        .filter_map(|v| {
                            Some((
                                v.get("SystemAddress")?.as_u64()?,
                                v.get("StarSystem")?.as_str()?.to_string(),
                                v.get("StarType")?.as_str()?.to_string(),
                            ))
                        })
                        .collect()
                })
...
pub fn load_star_overrides_into(galaxy: &ed_galaxy::Galaxy, conn: &rusqlite::Connection) -> usize {
    ...
    for (id64, class, subtype) in rows {
        let class = ed_galaxy::StarClass::from_name(&class).unwrap_or_else(|| {
            ed_galaxy::StarClass::from_subtype(subtype.as_deref().unwrap_or(""))
        });
        if class != ed_galaxy::StarClass::Unknown {
...
{"id64":44,"name":"Scanned Tauri","coords":{"x":140,"y":0,"z":0},"bodies":[]}
    ...
    fn journal_learned_stars_survive_a_restart() {
        ...
        store_star_override(
            ...
            "Scanned Neutron",
            "N",
            ed_galaxy::StarClass::from_journal("N"),
            "journal",
        )
        .unwrap();
        store_star_override(
            ...
            "Scanned Dwarf",
            "DA",
            ed_galaxy::StarClass::from_journal("DA"),
            "journal",
        )
        .unwrap();
        store_star_override(
            ...
            "Scanned Tauri",
            "TTS",
            ed_galaxy::StarClass::from_journal("TTS"),
            "journal",
        )
        .unwrap();
        assert_eq!(
            class_of(&galaxy, "Scanned Neutron"),
            ed_galaxy::StarClass::Unknown,
            "nothing learned yet"
        );
        ...
        assert_eq!(
            class_of(&galaxy, "Scanned Neutron"),
            ed_galaxy::StarClass::Neutron
        );
        assert_eq!(
            class_of(&galaxy, "Scanned Dwarf"),
            ed_galaxy::StarClass::WhiteDwarf
        );
        assert_eq!(
            class_of(&galaxy, "Scanned Tauri"),
            ed_galaxy::StarClass::Proto
        );
    }
    ...
    fn override_rows_persist_the_enum_name() {
        ...
        store_star_override(
            ...
            "Spansh Neutron",
            "Neutron Star",
            ed_galaxy::StarClass::from_subtype("Neutron Star"),
            "spansh /system",
        )
        .unwrap();
        let neutrons: i64 = conn
            .query_row(
                "SELECT count(*) FROM star_overrides WHERE class = 'Neutron'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(neutrons, 2);
        let scoopable: i64 = conn
            .query_row(
                "SELECT scoopable FROM star_overrides WHERE id64 = 42",
                [],
                |r| r.get(0),
            )
            .unwrap();
...
#[tauri::command]
pub async fn import_spansh_route(
    ...
    if status != "ok" {
        return Err(format!(
            "Spansh job is {status:?} ({}) -- wait for it to finish and try again",
    ...
    for j in jumps {
        ...
        let (mut class, idx, id64) = galaxy
            ...
            .unwrap_or((
                ed_galaxy::StarClass::Unknown,
                u32::MAX,
                j.get("id64").and_then(|x| x.as_u64()).unwrap_or(0),
            ));
...
mod tests {
    ...
    fn job_ids_are_extracted_from_links() {
        assert_eq!(
            job_id(
                "https://spansh.co.uk/exact-plotter/results/12345678-ABCD-11F1-9A6A-000000000000"
            )
            .as_deref(),
            Some("12345678-ABCD-11F1-9A6A-000000000000")
        );
        assert_eq!(
            job_id("12345678-abcd-11f1-9a6a-000000000000").as_deref(),
            Some("12345678-ABCD-11F1-9A6A-000000000000")
        );
