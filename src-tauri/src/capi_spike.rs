...
pub async fn start(app: tauri::AppHandle) -> Result<String, String> {
    ...
    *PENDING.lock().unwrap_or_else(|e| e.into_inner()) =
        Some(Pending { verifier, state: state.clone() });
    let url = format!(
...
async fn exchange_and_probe(
    ...
    let customer_id = decoded
        .pointer("/usr/customer_id")
    ...
    tracing::info!(status = decode_status, ms = decode_ms, ?fid_matches, "capi spike: /decode");
