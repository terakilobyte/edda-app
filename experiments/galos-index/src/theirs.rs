//! Headless copy of `galos_map/src/systems/route/graph.rs` at commit
//! `a8fc2fc` (galos, MIT, Nathan Lilienthal): the jump-route graph and its
//! hand-written A*. The search code is unchanged.
//!
//! What was done to make it run without bevy, and nothing else:
//!
//! - `bevy::math::DVec3` is `glam::DVec3` (the same type; bevy re-exports
//!   glam). `#[derive(Resource)]` and `use bevy::prelude::*` are gone, and
//!   with them the `Jumps(Arc<JumpGraph>)` resource wrapper.
//! - `crate::Boosts` is a local [`Boosts`] of the same shape (an
//!   `Arc<HashMap<i64, Boost>>` and a `published` flag) with the same `get`
//!   and `published`, plus [`Boosts::from_map`].
//! - The live search picture (`Frontier` / `Reached` / `Drawn` / `Sampler`,
//!   which the map drew a frame at a time) is reduced to a stub with the
//!   same names and the same call signatures, so [`JumpGraph::search`] is
//!   textually the original. The stub does one thing: [`Sampler::expanded`]
//!   counts, which is exactly where the original counted (`self.expanded +=
//!   1`, the first line of its body); the cells, edge, chain and coarsening
//!   are not kept, nor are the `cell_of` / `middle` / `coarser` helpers that
//!   served only them. That count is what [`Found::expansions`] reports.
//!   Note the original's count also did per-expansion work when a frontier
//!   was attached (a distance to the goal and, on a new closest, a walk up
//!   the parent map); the stub does not, so timings here are the search's
//!   own.
//! - Their `JumpGraph::new(&[NameEntry], &Boosts)` is kept under the name
//!   [`JumpGraph::of_entries`], since the bench's [`JumpGraph::new`] takes a
//!   prebuilt [`Places`]. `Places::of` (their constructor, from an iterator
//!   of `(i64, [f64; 3])`) is kept verbatim; [`Places::build`] feeds it from
//!   `(i64, [f32; 3])` without a `NameEntry` ever existing.
//! - The `route` free function at the bottom is the bench's entry: it calls
//!   their `JumpGraph::route` with a frontier attached and packs the answer
//!   into [`Found`].
//!
//! Tests: kept, save one. `a_search_that_spreads_is_held_at_the_cell_ceiling`
//! tests the frontier's coarsening against `super::frontier` constants and
//! is dropped with the picture. The rest are textually the originals except
//! that `JumpGraph::new(&entries, &boosts)` reads `JumpGraph::of_entries(...)`.
//! One test is added at the end, for the bench's own entry.
//!
//! Guesses: addresses are `i64` throughout (their `NameEntry::address` and
//! `Boosts` key are `i64`); `range` is light years, unaided, as their form
//! takes it (positions are in light years and the estimate divides a
//! light-year distance by it).
#![allow(dead_code)]
...
///
/// Stands in for `galos_map::Boosts`: the same two fields and the same two
/// readers the graph asks for.
...
impl Boosts {
    ...
    /// A table from what is already keyed by address.
    pub fn from_map(map: HashMap<i64, Boost>) -> Boosts {
        Boosts { by_address: Arc::new(map), published: true }
    }
...
/// be lying about what it plotted.
///
...
/// What a search has reached, as it reaches it — the headless stub
///
/// The original is the picture the map draws a frame at a time: the cells
/// the search has expanded in, the last few of them, and the chain to the
/// closest system reached, all under a lock. None of that is drawn here.
/// What is kept is the shape — the same names and signatures, so the search
/// below is the original text — and the one number the bench reads, how
/// many systems were expanded.
#[derive(Default)]
...
impl Frontier {
    ...
    ///
    /// The ends sized the picture's cells; nothing is drawn here and they
    /// are taken for the signature alone.
    pub(crate) fn between(_from: DVec3, _goal: DVec3) -> Arc<Frontier> {
        Arc::new(Frontier::default())
    }

    /// A sampler that feeds this, for a search to carry.
    pub(crate) fn sampler(self: &Arc<Frontier>) -> Sampler {
        Sampler { into: Arc::clone(self), expanded: 0 }
    }
...
/// A search's own tally, flushed into a [`Frontier`] when the search is done
///
/// The original sampled cells and the chain to the closest system reached
/// on top of the count; this counts.
...
impl Sampler {
    ...
    ///
    /// The same signature as the original's, which the search calls with the
    /// place, the goal, its parent map and a way to place any index; here
    /// only the count is kept, and it is the original's first line.
...
impl Places {
    ...
    /// Bucket `entries` straight from an iterator of address and position in
    /// light years, at the table's own precision — what [`placed`] does to a
    /// [`NameEntry`], without the entry.
    pub fn build(entries: impl Iterator<Item = (i64, [f32; 3])>) -> Places {
        Places::of(entries.map(|(address, p)| {
            (address, [p[0] as f64, p[1] as f64, p[2] as f64])
        }))
    }

    /// How many systems are held.
...
impl JumpGraph {
    /// The graph over a prebuilt base, with nothing named since.
    pub fn new(places: Places, boosts: Boosts) -> JumpGraph {
        JumpGraph { base: Arc::new(places), fresh: Arc::default(), boosts }
    }

    /// Build the graph from the resident names table and the supercharge
    /// table beside it. (Their `JumpGraph::new`.)
    pub fn of_entries(entries: &[NameEntry], boosts: &Boosts) -> JumpGraph {
        JumpGraph {
    ...
    ///
    /// Whole rather than added to, since the fresh set is itself the
    ...
    /// has reached. [`None`] where nothing is
...
/// What the bench reads off a route.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Found {
    /// How many jumps the route is: the hops less one.
    pub jumps: usize,
    /// The systems passed through, by address, start first.
    pub path: Vec<i64>,
    /// How many systems the search expanded — popped from the heap and had
    /// their neighbours enumerated. The start counts; the goal, whose pop
    /// returns before its neighbours are asked for, does not; a dearer
    /// duplicate skipped on the way out of the heap does not.
    pub expansions: u64,
}

/// Their [`JumpGraph::route`], with a frontier attached so the expansions
/// are counted, packed for the bench.
///
/// `range` is the ship's unaided jump range in light years; `start` and
/// `end` are system addresses. [`None`] where either end is unknown to the
/// graph or no chain of jumps connects them.
pub fn route(
    graph: &JumpGraph,
    start: i64,
    end: i64,
    ...
    let watching = Arc::new(Frontier::default());
    let path = graph.route(start, end, range, how, drive, Some(&watching))?;
    Some(Found {
        jumps: path.len() - 1,
        path: path.into_iter().map(|(address, _)| address).collect(),
        expansions: watching.expanded(),
    })
}
...
mod tests {
    ...
    fn a_range_too_small_to_estimate_does_not_overflow() {
        ...
        let graph = JumpGraph::of_entries(&entries, &boosts);
    ...
    fn a_route_of_equal_jumps_follows_the_straighter_chain() {
        ...
        let graph = JumpGraph::of_entries(&entries, &Boosts::default());
    ...
    /// took the long way round — which is what the refresh hands the
    /// arrivals here for.
    #[test]
    fn a_system_named_since_is_routable() {
        ...
        let graph = JumpGraph::of_entries(&base, &Boosts::default());
        for how in BOTH {
            assert!(
    ...
    // ---- The bench's own entry, over the same graph (added in the port) ----

    /// [`Places::build`] and the free [`route`] agree with their constructor
    /// and their `route`, and the expansions are counted.
    #[test]
    fn the_bench_entry_matches_their_route_and_counts_expansions() {
        let entries = vec![
            at(0, [0., 0., 0.]),
            at(1, [450., 0., 0.]),
            at(2, [900., 0., 0.]),
        ...
        let theirs = JumpGraph::of_entries(&entries, &Boosts::default());
        let places =
            Places::build(entries.iter().map(|e| (e.address, e.position)));
        assert_eq!(places.len(), 4);
        let ours = JumpGraph::new(places, Boosts::absent());

        for how in [Routing::Quick, Routing::Direct, Routing::Shortest] {
            let want: Vec<i64> = theirs
                .route(0, 9, 500., how, Drive::Unaided, None)
                .expect("their route")
                .into_iter()
                .map(|(a, _)| a)
                .collect();
            let found = route(&ours, 0, 9, 500., how, Drive::Unaided)
                .expect("a route");
            assert_eq!(found.path, want, "{how:?}");
            assert_eq!(found.jumps, 3, "{how:?}");
            // A straight line: the start and two waypoints expanded, the goal
            // popped and returned without being expanded.
            assert_eq!(found.expansions, 3, "{how:?}");
        }

        // Same system: one hop, nothing expanded.
        let found = route(&ours, 0, 0, 500., Routing::Direct, Drive::Unaided)
            .expect("a route to itself");
        assert_eq!((found.jumps, found.expansions), (0, 0));
        // An end the graph has never heard of is no route.
        assert!(
            route(&ours, 0, 42, 500., Routing::Direct, Drive::Unaided).is_none()
        );
    }
}
