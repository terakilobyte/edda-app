...
const MIGRATIONS: &[(&str, &str)] = &[
    ("0001_service_skeleton.sql", include_str!("../migrations/0001_service_skeleton.sql")),
    ("0002_eddn.sql", include_str!("../migrations/0002_eddn.sql")),
    ("0003_publications.sql", include_str!("../migrations/0003_publications.sql")),
    ("0004_station_system_cascade.sql", include_str!("../migrations/0004_station_system_cascade.sql")),
    ("0005_stars.sql", include_str!("../migrations/0005_stars.sql")),
    ("0006_commodity_metadata.sql", include_str!("../migrations/0006_commodity_metadata.sql")),
    ("0007_station_identity.sql", include_str!("../migrations/0007_station_identity.sql")),
    ("0008_feedback.sql", include_str!("../migrations/0008_feedback.sql")),
    ("0009_commodity_merge.sql", include_str!("../migrations/0009_commodity_merge.sql")),
    ("0010_knowledge.sql", include_str!("../migrations/0010_knowledge.sql")),
    ("0011_publication_cost.sql", include_str!("../migrations/0011_publication_cost.sql")),
    ("0012_market_freshness_index.sql", include_str!("../migrations/0012_market_freshness_index.sql")),
    ("0013_system_identity.sql", include_str!("../migrations/0013_system_identity.sql")),
...
pub async fn database_pool(config: &ServiceConfig) -> Result<PgPool> {
    ...
    ::metrics::histogram!("edda_startup_phase_seconds", "phase" => "connect").record(started.elapsed().as_secs_f64());
    let started = std::time::Instant::now();
    let applied = migrate(&pool).await?;
    ::metrics::histogram!("edda_startup_phase_seconds", "phase" => "migrations").record(started.elapsed().as_secs_f64());
    tracing::info!(
        applied,
...
async fn migrate(pool: &PgPool) -> Result<u64> {
    ...
    let mut lock = pool.acquire().await.context("failed to acquire a connection for migrations")?;
    ...
    .await;
    let _ = sqlx::query("SELECT pg_advisory_unlock($1)").bind(MIGRATION_LOCK).execute(&mut *lock).await;
    outcome
}
