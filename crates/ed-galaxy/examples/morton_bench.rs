...
fn cell_box(
    ...
    let lo = ed_galaxy::format::cell_of_with(
        [
            a[0].min(b[0]) - pad,
            a[1].min(b[1]) - pad,
            a[2].min(b[2]) - pad,
    ...
    let hi = ed_galaxy::format::cell_of_with(
        [
            a[0].max(b[0]) + pad,
            a[1].max(b[1]) + pad,
            a[2].max(b[2]) + pad,
...
fn probe_all(g: &ed_galaxy::Galaxy, lo: (i32, i32, i32), hi: (i32, i32, i32), reps: u32) -> Sample {
    ...
    Sample {
        cells,
        stars,
        micros: started.elapsed().as_secs_f64() * 1e6 / reps as f64,
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    let dir = std::path::PathBuf::from(
        args.next()
            .expect("usage: morton_bench <index_dir> [--boost <dir>]"),
    );
    ...
    let boost_g = match boost {
        Some(boost_dir) => {
            if !ed_galaxy::Galaxy::exists(&boost_dir) {
                eprintln!(
                    "building 250 ly highway sub-index at {}...",
                    boost_dir.display()
                );
                let started = Instant::now();
            ...
            eprintln!(
                "boost sub-index: {} systems, cell {} ly",
                opened.count, opened.cell_ly
            );
    ...
    if let Some((sub, sub_dir)) = boost_g {
        ...
        let scan = |center: [f32; 3], radius: f32, want: u8| -> bool {
            ...
            sub.for_each_within(center, radius, |idx, _| {
                ...
                let flags = ((class == ed_galaxy::StarClass::Neutron) as u8
                    * ed_galaxy::agg::ANY_NEUTRON)
                    | ((class == ed_galaxy::StarClass::WhiteDwarf) as u8
                        * ed_galaxy::agg::ANY_WHITE_DWARF)
                    | (((class.scoopable()
                        || sub.flags(idx) & ed_galaxy::format::FLAG_SCOOP_NEARBY != 0)
                        as u8)
        ...
        for (region, a, b) in corridors {
            for radius in [600.0f32, 1_200.0] {
                for (label, want) in [
                    ("scoop", ed_galaxy::agg::ANY_SCOOPABLE),
                    ("neutron", ed_galaxy::agg::ANY_NEUTRON),
                ] {
                    const STEPS: u32 = 200;
                    let at = |i: u32| -> [f32; 3] {
                    ...
                    assert_eq!(
                        scan_hits, oracle_hits,
                        "oracle disagrees with the scan in {region}"
                    );
                    println!(
                        ...
                        scan_us,
                        oracle_us,
                        scan_us / oracle_us.max(0.001),
                        nodes as f64 / STEPS as f64,
                        oracle_hits,
                        STEPS
                    );
                }
    ...
    println!(
        "{:<12} {:<18} {:>7} {:>12} {:>12} {:>12} {:>12} {:>8}",
        "index", "region", "pad_ly", "box_cells", "occupied", "probe_us", "walk_us", "speedup"
    );
    for (index_name, galaxy) in &indexes {
        ...
        for (region, a, b) in regions {
            for pad in [250.0f32, 500.0, 1_000.0] {
                ...
                let volume =
                    (hi.0 - lo.0 + 1) as u64 * (hi.1 - lo.1 + 1) as u64 * (hi.2 - lo.2 + 1) as u64;
                let reps = if volume > 2_000_000 {
                    1
