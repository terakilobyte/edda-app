...
fn main() -> anyhow::Result<()> {
    let dir = std::path::PathBuf::from(std::env::args().nth(1).expect("usage: sidecars <subindex_dir>"));
    ...
    eprintln!("graph250.bin in {:.1}s ({} cells, {} edges)", t.elapsed().as_secs_f64(), graph.leaf_count(), graph.edge_count());
    ...
    eprintln!("alt250.bin in {:.1}s ({} landmarks)", t.elapsed().as_secs_f64(), alt.landmarks().len());
    Ok(())
}
