...
fn main() -> anyhow::Result<()> {
    ...
    let alt = sub.alt().ok_or_else(|| anyhow::anyhow!("no alt250.bin beside the sub-index"))?;
    let a = g.pos_of(g.find(&from).ok_or_else(|| anyhow::anyhow!("unknown {from}"))?);
    let b = g.pos_of(g.find(&to).ok_or_else(|| anyhow::anyhow!("unknown {to}"))?);
    let goal_cell = sub
        .cell_index_of_pos(b)
        .ok_or_else(|| anyhow::anyhow!("{to} is not in an occupied highway cell; probing the nearest instead"))?;
    println!("{:>6} {:>12} {:>12} {:>8}", "t%", "euclid_ly", "alt_ly", "binds");
    ...
    for i in 0..=20 {
        ...
        println!("{:>5}% {:>12.0} {:>12.0} {:>8}{}", i * 5, euclid, bound, if binds { "YES" } else { "" }, note);
    }
