...
fn main() -> anyhow::Result<()> {
    ...
    let injection: Option<(f32, &'static str, u32)> = args
        .iter()
        .position(|a| a == "--inject")
        .and_then(|i| {
            Some((
                args.get(i + 1)?.as_str(),
                args.get(i + 2)?.parse::<u32>().ok()?,
            ))
        })
        .and_then(|(grade, n)| {
    ...
    let census_path = args
        .iter()
        .position(|a| a == "--census")
        .and_then(|i| args.get(i + 1))
        .cloned();
    let start_fuel: Option<f32> = args
        .iter()
        .position(|a| a == "--fuel")
        .and_then(|i| args.get(i + 1))
        .and_then(|v| v.parse().ok());
    ...
    if !Galaxy::exists(&ndir) {
        eprintln!(
            "building neutron sub-index with {} ly cells...",
    ...
    let (model, boost) = match ship {
        ...
        "explorer86" => (
            FuelModel::from_loadout(1323.3, 128.0, 6.8, 8, true, true, 86.0, 10.5, 0.0),
    ...
    let boost = if args.iter().any(|a| a == "--white-dwarfs") {
        boost
    } else {
        BoostProfile {
            white_dwarf: 1.0,
    ...
    let stop_weight: f32 = if try_hard {
        0.0
    } else {
        args.iter()
            .position(|a| a == "--stop-weight")
            .and_then(|i| args.get(i + 1))
            .and_then(|v| v.parse().ok())
            .unwrap_or(1.0)
    };
    let req = RouteRequest {
        ...
        boost,
        fuel: Some(model),
        start_fuel: start_fuel.unwrap_or(model.capacity),
        thorough: thorough || try_hard,
    ...
    let stage = move |s: &str, i: u32, n: u32| {
        if s != "refine" || i == 0 || i == n {
            eprintln!("  stage {s} {i}/{n} at {} ms", t0.elapsed().as_millis())
        }
    };
    let found = move |r: &ed_galaxy::router::Route| {
        eprintln!(
            "  found {} jumps at {} ms",
            r.jumps,
    ...
    let trace_census =
        |phase: &'static str, pos: [f32; 3], _: f32| census.lock().unwrap().push((phase, pos));
    let trace_off = |_: &'static str, _: [f32; 3], _: f32| {};
    let trace: &(dyn Fn(&'static str, [f32; 3], f32) + Sync) = if census_path.is_some() {
        &trace_census
    } else {
        &trace_off
    };
    ...
    let r = if exact {
        ...
        plan_best(&g, Some(&neutrons), &req, &ctl)?
    };
    ...
    let est_refuel_s = r.refuel_stops as f32 * 36.0
        + if model.scoop_rate > 0.0 {
            refuel_tonnes / model.scoop_rate
        } else {
            r.refuel_stops as f32 * 95.0
        };
