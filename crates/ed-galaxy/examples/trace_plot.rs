...
fn main() -> anyhow::Result<()> {
    ...
    let req = match args.get(3) {
        Some(r) => RouteRequest { from, to, range_ly: r.parse()?, ..Default::default() },
        None => {
            let model = FuelModel::from_loadout(1323.3, 128.0, 6.8, 8, true, true, 77.81, 10.5, 0.0);
            let boost = if white_dwarfs { BoostProfile::MK2_SCO } else { BoostProfile { white_dwarf: 1.0, ..BoostProfile::MK2_SCO } };
            RouteRequest { from, to, range_ly: model.range_at(model.capacity), supercharge: true, boost, fuel: Some(model), start_fuel: model.capacity, ..Default::default() }
        }
    };
    ...
    let events: Mutex<Vec<(std::thread::ThreadId, &'static str, [f32; 3], f32)>> = Mutex::new(Vec::new());
    let trace = |phase: &'static str, pos: [f32; 3], v: f32| events.lock().unwrap().push((std::thread::current().id(), phase, pos, v));
    let ctl = Control { cancelled: &|| false, progress: &|_, _| {}, stage: &|_, _, _| {}, found: &|_| {}, trace: &trace };
    let route = plan_long(&g, &neutrons, &req, &ctl)?;
    eprintln!("{} jumps, {:.0} ly, {} boosted, {} refuels, {} ms", route.jumps, route.total_ly, route.boosted_jumps, route.refuel_stops, route.elapsed_ms);
    ...
    let on_route = |q: &[f32; 3]| route.hops.iter().any(|h| (h.pos[0] - q[0]).abs() < 0.2 && (h.pos[1] - q[1]).abs() < 0.2 && (h.pos[2] - q[2]).abs() < 0.2);
    let mut winner: Option<std::thread::ThreadId> = None;
    let mut tids: Vec<std::thread::ThreadId> = all.iter().filter(|e| e.1 == "chain").map(|e| e.0).collect();
    tids.dedup();
    for t in tids {
        let chain: Vec<&[f32; 3]> = all.iter().filter(|e| e.0 == t && e.1 == "chain").map(|e| &e.2).collect();
        if !chain.is_empty() && chain.iter().all(|q| on_route(q)) {
            winner = Some(t);
            break;
        }
    }
    let winner = winner.ok_or_else(|| anyhow::anyhow!("no variant's chain matches the final route"))?;
    let mut seen_leg = std::collections::HashSet::new();
    let mut seen_exact = std::collections::HashSet::new();
    let key = |q: &[f32; 3], v: f32| ((v * 10.0) as i64, (q[0] * 10.0) as i64, (q[1] * 10.0) as i64, (q[2] * 10.0) as i64);
    ...
    eprintln!("trace: {} coarse, {} chain, {} exact, {} leg hops", counts("coarse"), counts("chain"), counts("exact"), counts("leg"));
    ...
    let seg_dist = |q: [f32; 3]| -> f32 {
        ...
        let t = if len2 > 0.0 { ((aq[0] * ab[0] + aq[1] * ab[1] + aq[2] * ab[2]) / len2).clamp(0.0, 1.0) } else { 0.0 };
    ...
    eprintln!("{} neutrons within {half_width:.0} ly of the corridor", near.len());
    let ev: Vec<String> = events.iter().map(|(p, q, v)| format!("[\"{p}\",{:.1},{:.1},{:.1},{v:.0}]", q[0], q[1], q[2])).collect();
