...
export const topics = [
  ...
  },
  {
    ...
    body: [
      ...
      "Stacking mode's board now carries the stack's figures under the givers, every one a stated field summed, never an estimate: the kills that clear the stack (the largest per-giver sum, since one giver's missions progress one after another) and how many are still to make, every kill count added (what one kill is worth in mission credit, all givers together), the stack's value, what is ready to collect and what a wing shares. When you dock where missions are ready to hand in, the tab and the HUD show how many and for how much, and the ship computer says it once per dock; the paid reward replaces the offer in the history once the game pays. These two ideas come from ODEliteTracker, whose approach to mission stacks we studied; no code was copied.",
    ],
  ...
  },
  {
    ...
    body: [
      ...
      "Plan build opens the Build planner on that ship — or pick any hull under \"Any ship\" to plan a ship you do not own yet, from its stock fit. Every slot is a row, empty ones too, and Swap to offers only what that slot takes: the right sizes, utility mounts for utility modules, military slots for hull and module reinforcement and cell banks, life support and sensors at exactly the slot's size, fighter hangars and luxury cabins only on the hulls that sell them. Every slot but a core one can be emptied. A swapped module starts its engineering from grade 0 — except the brokers' pre-engineered modules (the SCO drive V1, with Increased Range and Fast Boot at grade 5 already on), which come at their grade with their fixed figures and leave only an experimental to plan. The figures, the material list and the SLEF export all carry the swaps. The planner turns the module table into a plan: a blueprint, target grade and experimental for every engineerable module, \"same for all N\" to give every module of one type the same plan, saved per ship. Fitted engineering continues to the top grade unless you change it; a module at the top grade with no experimental has nothing to do and cannot be ticked. Materials for this build gives one answer for the lot: every material pooled against what you carry, one shopping list for the shortfall with the nearest traders and farm sites, and the fewest engineers to visit with what to bring each. What no unlocked engineer can do at the asked grade is grouped, with who takes it part-way today and who to unlock for the rest. Every engineer who works a blueprint is listed with the grade they reach (\"The Dweller · rank 5 · to G4\"), so one who stops a grade short is never hidden; the engineer table is checked against Inara and the wiki and held by a test. The shopping list knows the material-trader rates (6:1 per grade up, 3:1 per grade down, 6:1 across groups) and never offers Guardian or Thargoid materials as trades. All of it is local: the bundled blueprint data and your own journal.",
      "Import a build (on the Build planner): paste an EDSY or Coriolis SLEF export (Export → SLEF on either site) and the plan becomes the difference between your ship and that build — the modules to swap (they land in the Swap to column), then every engineering job to reach it, continuing from what is already rolled. On a hull you do not own, the gap is from the stock fit. A build that puts a module where the hull has no such slot, or one the slot does not take, is said so and left out. Coriolis's JSON export names modules its own way and is refused with the way out.",
