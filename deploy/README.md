...
VictoriaMetrics (127.0.0.1:8428, scrapes ed-api /metrics plus
prometheus-node-exporter on :9100 and prometheus-postgres-exporter on
:9187 — the Host Health dashboard and the Server dashboard's Postgres
row read nothing else, and read "No data" until the 2026-09-20 audit
added them; 180 d retention) + Grafana OSS (127.0.0.1:3000) — ALL loopback-only: the
...
CI's `edda-apply dashboards` installs JSON-only changes, anything touching
vm-scrape.yml, the units or the exporters needs `setup-monitoring.sh`
re-run as root — it is idempotent). Panel-by-panel no-data triage is
`docs/benches/knobs/dashboard_audit.py` (static from the repo; live with
GRAFANA_USER/GRAFANA_PASSWORD through the tunnel). The feedback browse reads Postgres through grafana_ro
